...
class Visualizer(Callback):
    ...
    def record_pr_curve(self):
        ...
        for type_, data_ in data.items():
            ...
            with self.writer.as_default():
                result = tf.summary.experimental.write_raw_pb(summary_pb.SerializeToString(), step=self.get_current_step())
    ...
    def prepare_internal_metrics(self):
        ...
        self.per_epoch_resources['pr_curve'] = {
            'pixel': {
                ...
                #'FBetaScore': custom_metrics.FBetaScore(1,pixel_thresholds), summary_libに対応していない　そもそも必要ない？
            },
        ...
        for type_ in self.per_epoch_resources['pr_curve']:
            if type_ == 'pixel':
                for metric in self.internal_metics:
                     ...
                     #　上のmetricsを対応させるにはそれぞれ定義しなおすか、ここでチャンネルごと計算する
    ...
    def process_batch(self, batch):
        ...
        self.update_internal_metrics(batch[1], batch_output)
        ...
        # else:
        # sensitivity_images = [None] * len(consts)
        # sensitivity_data = [None] * len(consts)
        #
        ...
        #     ]
        #     assert len(metric_data) == len(consts)
        # else: metric_data = [None] * len(consts)
        #
        # with ThreadPoolExecutor() as e:
    ...
    def _emit(self, tag, image, sensitivity_image=None, sensitivity_data=None, metrics_data=None):
        ...
        #     path = os.path.join(self.save_dir, 'images', f'step_{step:08d}.png')
        ...
        #             self.save_dir, 'csv', f'step_{step:08d}_metrics.csv')
    ...
    def make_summary_batch(self, batch, batch_output, overlay=False):
        results = tf.map_fn(
            lambda x: self.make_summary_constructor(x[0], x[1], x[2], overlay=overlay),
            (batch[0], batch[1], batch_output),
            dtype=tf.float32,
    ...
    @tf.function
    def make_summary_constructor(self, features, label, output, overlay=False):
